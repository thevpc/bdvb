...
function update_document_for_loaded_packets() {
    ...
    if (my_module.server_packets.length > 0) {
        ...
        $("#box3Val").text("Status");
        $("#box3Label1").text("Mileage : " + r0.Mileage + " km");
...
function uniformRow(r0, aggregate_alarms) {
    ...
    try {
        ...
        if (o.Speed != 0) {
            ...
            o.ActiveAlarms.push(extra_alarm_moving.name);
        }
...
function add_alarm(o, rr, visited) {
    ...
    for (var p in pp) {
        if (pp.hasOwnProperty(p)) {
            ...
            var v = pp[p];
            if (n.startsWith('Alarm ')) {
                var oo = {};
                oo.name = n.substring(6);
                oo.value = parseInt(v);
                ...
                if (oo.value == 1) {
                    if (!visited["a!:" + oo.name]) {
                        visited["a!:" + oo.name] = true;
                        o.ActiveAlarms.push(oo.name);
                    }
                }
            } else if (n.startsWith('Status ')) {
                var oo = {};
                oo.name = n.substring(7);
                oo.value = parseInt(v);
                if (!visited["s:" + oo.name]) {
                    visited["s:" + oo.name] = true;
                    o.Statuses.push(oo);
                }
                if (oo.value == 1) {
                    if (!visited["s!:" + oo.name]) {
        ...
        r0.Alarms.forEach(function (ii) {
            if (ii.value == 0) {
                ...
                eventArray.push(createEvent(eventIdLabel, {
                    ...
                    icon: 'fa-clock-o',
                    name: 'alarm'
                }, ii.name, [{type: 'danger', icon: 'fa-clock-o', name: 'fired'}]));
            }
        });
    }
